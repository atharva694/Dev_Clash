    ...
    try {
      const res = await fetch("/api/refine", {
        ...
        body: JSON.stringify({
          ...
          optimized_prompt: result.optimized_prompt || originalPrompt
        }),
      });
      ...
      if (!res.ok) {
        throw new Error(data.detail || data.error || "Failed to refine code");
      }

      // We merge iterations from previous result with the new one
      const newData = data as GenerateResponse;
      if (result.total_iterations && newData.total_iterations) {
         newData.total_iterations += result.total_iterations;
      }
      setResult(newData);
      setRefineFeedback("");
    } catch (err: any) {
      alert(err.message || "Network error while refining.");
    } finally {
      setIsRefining(false);
    }
  };
                    ...
                    <div className="w-[340px] flex flex-col gap-6 shrink-0">
                      <div className="p-6 rounded-2xl border border-white/5 bg-[#131825] shadow-xl">
                         ...
                         <textarea 
                           ...
                           placeholder="e.g. Make the header blue and add a dark mode toggle..."
                           disabled={isRefining}
                         />
                         <button 
                           onClick={handleRefine}
                           disabled={!refineFeedback.trim() || isRefining}
                           className="w-full py-3.5 rounded-xl bg-violet-600 hover:bg-violet-500 disabled:bg-violet-800 disabled:opacity-50 text-white text-sm font-semibold transition-all flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(124,58,237,0.3)] hover:shadow-[0_0_25px_rgba(124,58,237,0.4)]"
                         >
                           {isRefining ? (
                             <>
